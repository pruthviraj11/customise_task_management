$(function () {
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: false,
        url: ViewUrl/* assetPath + 'data/kanban.json' */,
    ...
    function renderDropdown(encryptedId) {
        // Define the base URL for the task edit link, replacing `:encrypted_id` in JavaScript
        var editTaskLink = "/app/task/edit/" + encryptedId;

        // Generate the dropdown HTML with the edit task link
        var dropdownHtml =
            "<a class='dropdown-item' target='_blank' href='" + editTaskLink + "'>Edit</a>" +
    ...
    var kanban = new jKanban({
        ...
        dropEl: function (el, target, source, sibling) {
            ...
            var StatusId = $(el).parents('.kanban-board').attr('data-id');
            var upDateUrlTemp = upDateUrl;
            $.ajax({
                type: 'GET',
                dataType: 'json',
                async: false,
                url: upDateUrlTemp.replace('taskId', taskId).replace('StatusId', StatusId),
                success: function (data) {
                    if (data.status === 1) {
                        Swal.fire({
                            icon: 'success',
    ...
    $.each($('.kanban-item'), function () {
        ...
        // Check for 'data-badge' and 'data-badge-text' attributes and render the header accordingly
        ...
        // Check for additional attributes and render the footer accordingly
        if (
        ...
        // Check for an image and modify the content to include the image and footer
        ...
        // Add the edit button
        var editButton = "<div class='d-flex'>  <a class='kanban-edit-btn btn-sm btn-warning me-1' data-id='" + $this.attr('data-eid') + "' > Edit</a> <a class='kanban-view-btn btn-sm btn-info me-1' data-id='" + $this.attr('data-eid') + "'>view</a></div>";
        // var viewButton = "<a class='kanban-view-btn btn-sm btn-warning me-1' data-id='" + $th.is.attr('data-eid') + "'>Edit</a>";

        // Append the edit button to the kanban item (either header or footer)
    ...
    $(document).on('click', '.kanban-edit-btn', function () {
        var encryptedId = $(this).data('id');

        // Construct the URL for the edit task route dynamically
        var editTaskUrl = '/app/task/edit/' + encryptedId;

        // Redirect to the edit task route
        window.location.href = editTaskUrl;
    });
    $(document).on('click', '.kanban-view-btn', function () {
        var encryptedId = $(this).data('id');

        // Construct the URL for the edit task route dynamically
        var editTaskUrl = '/app/task/view/' + encryptedId;
